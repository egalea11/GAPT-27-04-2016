...
namespace GAPT.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        private ToursDbContext tourdb = new ToursDbContext();
        ...
        {
            if (db.Users.Where(user => user.Email == model.Email).ToList().Count != 0)
            {
                ModelState.AddModelError("Email", "Email address already exists. Please enter another email address");
                return View();
            }
            if (db.Users.Where(user => user.UserName == model.UserName).ToList().Count != 0)
            {
                ModelState.AddModelError("UserName", "Username already exists. Please enter another Username");
                return View();
            }
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    await SendEmail(model.Email, "Confirm your account", "Please confirm your account by clicking <a href=\"" + callbackUrl + "\">here</a>");
                    var users = db.Users.ToList();
                    var adminUsers = db.Users.Where(u => u.IsAdmin == true).ToList();

                    //return RedirectToAction("Index", "Home");
                    return RedirectToAction("RegisterAccountConfirmation", "Account");
                }
                AddErrors(result);
            }
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                await SendEmail(model.Email, "Reset Password", "Please reset your password by clicking <a href=\"" + callbackUrl + "\">here</a>");
        ...
        public async Task<Boolean> SendEmail(string userEmail, string subject, string body)
        {
            if (ModelState.IsValid)
            {
